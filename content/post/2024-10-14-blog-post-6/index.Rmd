...
d_campaign_spending <- d_state_popvote |> 
  ...
  left_join(campaign_spending |>
              ...
              group_by(election_year, contribution_state) |>
              summarize(total_amount = sum(contribution_receipt_amount), .groups = 'drop'),
              by = c("year" = "election_year", "state_abb" = "contribution_state")) |>
  filter(year >= 2008)

d_campaign_spending

d_campaign_spending <- d_campaign_spending |>
  group_by(year, state) |>
  summarize(total_amount)
...
personal_dis_income <- personal_dis_income |>
  ...
  mutate(across(-state, ~ as.numeric(gsub("\\(NA\\)", NA, .)))) |>
  pivot_longer(
    cols = -c(state, GeoFips),  
    names_to = "Year", 
    values_to = "Disposable_Income" 
  ) |>
  mutate(year = as.numeric(Year))   |> #for some reason some years were characters so I made them all numbers
  mutate(state = gsub("\\*", "", state)) |>
  filter(year >= 2008) |>
  filter(!state %in% regions_to_remove & !is.na(state))  
...
# Merge data.\
d <- d_pollav_state |>
  full_join(d_state_popvote, by = c("year", "state")) |>
  full_join(d_popvote |> filter(party == "democrat"), by = "year") |>
  full_join(d_turnout, by = c("year", "state")) |>
  full_join(personal_dis_income, by = c("state", "year")) |>
  filter(year >= 1980) |>
  ungroup()
...
states.2024 <- unique(d_pollav_state$state[d_pollav_state$year == 2024])
states.2024
...
d.train <- d |> filter(year >= 2008) |> select(year, state, D_pv2p, latest_pollav_DEM, mean_pollav_DEM, 
                                              D_pv2p_lag1, D_pv2p_lag2, Disposable_Income) |> drop_na()
d.train

d
d.test <- d |> filter(year >= 2016) |> select(year, state, D_pv2p, latest_pollav_DEM, mean_pollav_DEM, 
...
t <- d |> 
  ...
  filter(year == 2024) |> 
  select(state, year, D_pv2p, R_pv2p, D_pv2p_lag1, R_pv2p_lag1, D_pv2p_lag2, R_pv2p_lag2, Disposable_Income) 

t

#add in lagged disposable income
...
electoral_outcomes <- win_pred |>
  group_by(state) |>
  summarize(
    mean_dem = mean(simp_pred_dem),
    mean_rep = mean(simp_pred_rep)
  ) |>
  mutate(sd_dem = sd(simp_pred_dem),
